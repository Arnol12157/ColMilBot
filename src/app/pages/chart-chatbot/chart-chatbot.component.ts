...
import {CHART_TEXT_COLOR, MAT_DEEP_ORANGE} from '../../utils/colors';
...
export interface Chatbot { conversacion: any[]; fecha_interaccion: any}

@Component({
  selector: 'app-chart-chatbot',
  templateUrl: './chart-chatbot.component.html',
  styleUrls: ['./chart-chatbot.component.scss'],
    animations: [routerAnimation]
})
export class ChartChatbotComponent implements OnInit, OnDestroy {
    ...
    resizeSubscription;
    // Chart model
    heatmapOption = this.createOption(null);

    private chatbotCollection: AngularFirestoreCollection<Chatbot>;
    itemsChatbot: Observable<Chatbot[]>;
    data: any[] = [];
    dates: any[] = [];
        ...
        for (var i=0; i<7; i++){
          for(var j=0; j<24; j++){
            ...
            if(j<10){
              ...
              var startDate = datePipe.transform(new Date().setDate(new Date().getDate()-i), 'dd/MM/yyyy '+hora+':00');
              var endDate = datePipe.transform(new Date().setDate(new Date().getDate()-i), 'dd/MM/yyyy '+hora+':59');
              this.chatbotCollection = afs.collection('chatbot', ref => ref.where("fecha_interaccion", ">=", startDate).where("fecha_interaccion", "<=", endDate));
              this.itemsChatbot = this.chatbotCollection.valueChanges();
              arrayCollections.push([i , j, this.itemsChatbot]);
          }
        }
        arrayCollections.forEach((aCollection, index) => {
            aCollection[2].subscribe((chatbot) => {
                this.data=chatbot;
                var array=[aCollection[0], aCollection[1], chatbot.length];
                this.dates.push(array);
                this.heatmapOption = this.createOption(this.dates);
            });
        })
    }
    ...
    createOption(aRaw: any) {
        const hours = ['12AM', '1AM', '2AM', '3AM', '4AM', '5AM', '6AM',
            '7AM', '8AM', '9AM', '10AM', '11AM',
            '12PM', '1PM', '2PM', '3PM', '4PM', '5PM',
            '6PM', '7PM', '8PM', '9PM', '10PM', '11PM'];
        const days = ['Hoy', 'Hace 1 dia', 'Hace 2 dias',
            'Hace 3 dias', 'Hace 4 dias', 'Hace 5 dias', 'Hace 6 dias'];
        ...
        const data = raw.map(function (item) {
        ...
        const option = {
            ...
            visualMap: {
                ...
                bottom: '15%',
                inRange: {
                    color: [MAT_DEEP_ORANGE._100, MAT_DEEP_ORANGE._500]
                },
                textStyle: {
                    color: CHART_TEXT_COLOR
                }
